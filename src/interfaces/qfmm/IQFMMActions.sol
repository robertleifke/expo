...
/// @title  Action functions for the QFMM contract
/// @author Robert Leifke
interface IQFMMActions {
    // ===== Pool Updates =====
    ...
    /// @param  tokenYPerLp  Risky reserve per liq. with risky decimals, = 1 - N(d1), d1 = (ln(S/K)+(r*σ^2/2))/σ√τ, valid for [0, 1e^(risky token decimals))
    ...
    /// @return delTokenY    Total amount of risky tokens provided to reserves
    /// @return delTokenX   Total amount of stable tokens provided to reserves
    // function create(
    //     uint128 strike,
    //     uint32 maturity,
    //     uint32 gamma,
    ...
    /// @param  delTokenY    Amount of risky tokens to deposit
    /// @param  delTokenX   Amount of stable tokens to deposit
    ...
    /// @param  delTokenY    Amount of risky tokens to withdraw
    /// @param  delTokenX  Amount of stable tokens to withdraw
    // function withdraw(
    //     address recipient,
    //     uint256 delTokenY,
    //     uint256 delTokenX
    ...
    /// @return delTokenY      Amount of risky tokens received from removed liquidity
    /// @return delTokenX     Amount of stable tokens received from removed liquidity
    // function remove(bytes32 poolId, uint256 delLiquidity) external returns (uint256 delTokenY, uint256 delTokenX);
